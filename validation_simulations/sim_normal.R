...
for(i in 1161:nrow(grid)) {
  ...
  s <- sdmTMB_sim(
    ...
    seed = grid$iter[i], family = gaussian()
  )
...
saveRDS(grid, "validation_simulations/normal_results.rds")
...
g1 = ggplot(data = dplyr::filter(grid,rho==0), aes(x=as.factor(b_trend), y=trend_est - b_trend)) +
  geom_boxplot(aes(colour = obs_sd), outlier.colour = NA) +
  theme_bw() +
  ylim(-0.1,0.1) +
  xlab("True trend") +
...
pdf("validation_simulations/sim_normal.pdf")
